...
N = source_data[0][0].shape[0] + source_data[1][0].shape[0]
wr = np.ones(N)
ws = np.ones(N)
...
algorithm = WFQI(target_mdp, pi, actions, batch_size = batch_size, max_iterations = max_iterations, regressor_type = ExtraTreesRegressor, source_datasets = source_data, var_rw = 0, var_st = 0, max_gp = 0,
...
experiment = RepeatExperiment("direct", algorithm, n_steps = n_steps, n_runs = n_runs, callback_list = callback_list)
